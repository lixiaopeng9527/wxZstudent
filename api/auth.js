...
static loginSys(){
     ...
     let wxUnionId = Storage.getStorageSync("wxUnionId");
      AuthSev.checkSession()
        .then(isWxLogin=>{     
          ...
          //已登录
          if (isWxLogin && session && wxUnionId){          
            console.log("已经登录.......");
            resolver({ session: session, wxUnionId: wxUnionId});
          }
          //未登录,开始登录
          else{
            console.log("未登录..");
            ...
            AuthSev.wxLogin()
              ...
              .then((res) => {
                ...
                return AuthSev.getCicadaSession(res.code);
              })
              //获得session
              .then((res) => {    
                console.log('session-->',res);            
                if (res.bizData.session) Storage.setStorageSync("wx_session", res.bizData.session);
                if (res.bizData.wxUnionId) Storage.setStorageSync("wxUnionId", res.bizData.wxUnionId);
   ...
   * 获得微信的openId
   */
  static getCicadaSession(code){  
    return Net.postCicadaJSON(Config.SERVER.url.qrcode + "/auth/wx/applet/login", { code: code, appCode: 'zs'},{});
  }
        ...
        .then((res) => {  
          ...
          // 这块说明已经绑定过了 
            ...
            if (!res.bizData.hasBindUnionId) {
              ...
              AuthSev.wxLogin()
                .then(tt => {
                  return AuthSev.getCicadaSession(tt.code);
                })
                .then(() => {
                  resolver(res.bizData);
                }, err => {
